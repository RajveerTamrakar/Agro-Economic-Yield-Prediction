...
This repository contains an end-to-end machine learning project designed to predict the income of farmers in India. The primary objective is to help financial institutions improve their credit assessment process for the unbanked agricultural population. By leveraging a rich dataset encompassing demographic, landholding, and climatic data, this project employs a sophisticated machine learning pipeline to generate accurate income predictions.

The solution is implemented in a Jupyter Notebook (`Agro.ipynb`) and uses a stacked ensemble of multiple gradient boosting and tree-based models to achieve high accuracy.

-----

## 🎯 Problem Statement

Access to credit is a significant challenge for individuals in the farming sector due to a lack of sufficient credit history. This often leads to exploitation by untrustworthy lenders. Financial institutions aims to address this by using alternative data sources to assess the creditworthiness of farmers. This challenge seeks to develop a model that can accurately predict farmer income, thereby minimizing the rejection of loan applications for deserving individuals.
...
The project also uses the `Agro Dataset.csv` which contains the final predictions.

-----

## 🛠️ Methodology

The solution follows a structured machine learning pipeline, as detailed in the `Agro.ipynb` notebook.

### 1\. Data Loading and Preparation

  * The training and test datasets are loaded from the provided Excel file (`Agro Dataset.csv`).
...
2.  Place the `Agro Dataset.csv` file in the same directory as the `Agro.ipynb` notebook.
3.  Open and run the `Agro.ipynb` notebook in a Jupyter environment like Jupyter Lab or Google Colab.
